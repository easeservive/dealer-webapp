...
for brand, cars in all_cars.items():
    ...
    for car in cars:
        ...
        unique_id = True
        while unique_id:
            sql_command = "SELECT `model` FROM `vehicles` WHERE `vehicle_model_id`=%s"
            connection.cursor().execute(sql_command, (vehicle_model_id))
            result = connection.cursor().execute(sql_command, ("assaas"))
            if result == 0:
                unique_id = False

        sql_command = "INSERT INTO `vehicles` (`vehicle_model_id`, `model`, `brand`, `vehicle_type`) VALUES (%s, %s, %s, %s)"
